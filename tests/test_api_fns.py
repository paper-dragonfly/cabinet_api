...
class TestInsert:
    def test_check_for_duplicate(self):
        ...
        with Session() as session:
            # new blob
            inst = StorageFnSchema(
                metadata={"blob_type": "fruit", "blob_hash": "myhash"},
    ...
    def test_generate_paths(self):
        ...
        new_blob_unsaved = StorageFnSchema(
            metadata={"blob_type": "fruit", "blob_hash": blob_hash},
            storage_envs=storage_envs,
        )
        paths = f.generate_paths(new_blob_unsaved)
        assert paths == {
            "blobs/test/fruit/fc278761b5697780831b090e61405942acf974f102824e58bcf025fda3f1e357",
            "blobs/fruit/fc278761b5697780831b090e61405942acf974f102824e58bcf025fda3f1e357",
            "gs://cabinet22_fruit/fc278761b5697780831b090e61405942acf974f102824e58bcf025fda3f1e357",
        }

        with pytest.raises(KeyError):
            StorageFnSchema(
                metadata={"blob_hash": blob_hash, "color": "red"},
                storage_envs=["testing"],
            )
    ...
    def test_add_blob_paths(self):
        ...
        paths = [
            "cabinet_api/blobs/a591a6d40bf420404a011733cfb7b190d62c65bf0bcda32b57b277d9ad9f146e"
        ]
        # pass to fn
        with Session() as session:
...
class TestSearch:
    ...
    def test_build_results_dict(self):
        ...
        assert resp == {
            ...
            "fruit_name": ["mango", "kiwi"],
            "fruit_color": ["orange", "green"],
            "blob_hash": ["mangohash", "kiwihash"],
        }
    ...
    def test_search_metadata(self):
        ...
        with Session() as session:
            ...
            session.commit()
        # assert expected
        assert f.search_metadata("fruit", self.valid1, session) == {
            "entry_id": [22],
            "blob_type": ["fruit"],
            "fruit_name": ["mango"],
            "fruit_color": ["orange"],
        ...
        assert f.search_metadata("fruit", {"blob_type": "fruit"}, session) == {
            "entry_id": [22, 23],
            "blob_type": ["fruit", "fruit"],
            "fruit_name": ["mango", "kiwi"],
            "fruit_color": ["orange", "green"],
...
class TestFnsUpdate:
    ...
    def test_update_fields(self):
        assert f.validate_update_fields("fruit", {"fruit_name": "mango"}) == {
            "valid": True
        }
        assert f.validate_update_fields("truck", {"fruit_name": "mango"}) == {
            "valid": False,
            "error": "BlobTypeError: truck is not a valid blob type",
        }
        assert f.validate_update_fields("fruit", {"fruit_age": "mango"}) == {
            "valid": False,
            "error": f"FieldError: invalid fields for blob_type fruit ['fruit_age']",
        }
    ...
    def test_make_full_update_dict(self):
        ...
        assert f.make_full_update_dict(update_dict, old_metadata) == {
            "blob_hash": "hash5",
            "fruit_color": "yellow",
            "fruit_name": "banana",
        }
